...
int vtkPolyDataNormalsCentroids::RequestData(vtkInformation *vtkNotUsed(info), vtkInformationVector ** inputVector, vtkInformationVector * outputVector)
{
  ...
  if (this->ComputePointCentroids)
  {
    ...
    for (vtkIdType i = 0; i < numNewPts; ++i)
    {
		...
		fCentroids[3 * i + 0] -= newPts->GetPoint(i)[0] / 3;
		fCentroids[3 * i + 0] /= 2;
		fCentroids[3 * i + 0] *= 3;
		fCentroids[3 * i + 1] /= counter[i];
		fCentroids[3 * i + 1] -= newPts->GetPoint(i)[1] / 3;
		fCentroids[3 * i + 1] /= 2;
		fCentroids[3 * i + 1] *= 3;
		fCentroids[3 * i + 2] /= counter[i];
		fCentroids[3 * i + 2] -= newPts->GetPoint(i)[2] / 3;
		fCentroids[3 * i + 2] /= 2;
		fCentroids[3 * i + 2] *= 3;
    }
	delete[] counter;
  ...
  //  splitting), we can simply pass data through, since newPts = inPts;
  //
